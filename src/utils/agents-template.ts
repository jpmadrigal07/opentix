...
/**
 * Build the .opentix/AGENTS.md content dynamically from project config.
 * This file is regenerated every time ensureOpentixStructure() runs
 * so it always stays in sync with config.yml.
 */
export function buildAgentsTemplate(config: OpentixConfig): string {
  const prefix = config.prefix;
  const statuses = config.statuses;
  const statusFlow = statuses.join(' -> ');
  const statusList = statuses.map((s) => `\`${s}\``).join(', ');
  const exampleId = `${prefix}-0001`;
  const exampleId2 = `${prefix}-0042`;

  return `<!-- AUTO-GENERATED by Opentix -- do not edit manually. -->
...
### Example Ticket (\`tickets/${exampleId}.md\`)
...
|---|---|---|---|
| \`id\` | string | Yes | Unique ticket ID (\`${prefix}-NNNN\`). Immutable after creation. |
| \`title\` | string | Yes | Short summary of the ticket. |
| \`status\` | string | Yes | Current status. One of: ${statusList}. |
...
- **Tickets**: \`${prefix}-NNNN\` (zero-padded 4-digit number, e.g., \`${exampleId}\`, \`${exampleId2}\`)
...
Valid statuses for this project: ${statusList}
...
| Field | Description |
|---|---|
| \`prefix\` | Ticket ID prefix (e.g., \`${config.prefix}\`). |
...
- Reference tickets by their ID (e.g., \`${exampleId}\`) in commit messages, comments, and documentation.
...
(pattern: \`${prefix}-NNNN\` in the branch name) and writes the full ticket
...
| Ticket by ID | \`.opentix/tickets/${prefix}-NNNN.md\` |
...
| Valid statuses | ${statusList} |
| ID pattern | \`${prefix}-NNNN\` (zero-padded) |
