...
Opentix syncs ticket data through Git. All `.opentix/` files (tickets, index, sprints, team registry) live on the dedicated `opentix` holder branch and are committed and pushed like any other file. The sync system runs in the background so your team always sees the latest state.
...
1. Pulls from the remote holder branch (`origin/opentix`) using merge-safe sync
...
When multiple team members edit tickets simultaneously, Git may encounter conflicts. Opentix handles this with a **merge-retry** strategy:

1. Pull with merge (no rebase)
2. If a conflict occurs, abort incomplete state and retry safely
